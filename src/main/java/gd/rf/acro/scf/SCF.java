...
public class SCF implements ModInitializer {
	...
	{
		File file = new File(FabricLoader.getInstance().getConfigDirectory().getPath() + "/SCF/funnel_blocks.acfg");
		File file2 = new File(FabricLoader.getInstance().getConfigDirectory().getPath() + "/SCF/funnel_blocks_weights.acfg");
		try
		{
			if(!file.exists())
			{
				FileUtils.writeLines(file, Arrays.asList(cRES));
			}
			ORES= FileUtils.readLines(file,"utf-8").toArray(ORES);

			if(!file2.exists())
			{
				FileUtils.writeLines(file2, Arrays.asList(cResRolls));
			}
			WEIGHTS= FileUtils.readLines(file2,"utf-8").toArray(WEIGHTS);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
